...
# Adding moments
* Add postgres url to envs and settings
* Serve media files
Create and link media folder:
``` bash
mkdir /var/lib/dokku/data/storage/dtb
dokku storage:mount dtb /var/lib/dokku/data/storage/dtb:/storage
```
Create nginx config:
``` bash
/home/dokku/myapp/nginx.conf.d/media.conf
```
with:
``` bash
location /media {
    alias /var/lib/dokku/data/storage/dtb;
}
```
